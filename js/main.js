document.addEventListener('DOMContentLoaded', () => {
  ...
  const addPhotoFigcaption = () => {
    ...
    document.querySelectorAll('#article-container micro').forEach(item => {
      const altValue = item.title || item.alt
      if (!altValue) return
      const ele = document.createElement('div')
      ele.className = 'micro-alt text-center'
   ...
   */
  const runLightbox = () => {
    btf.loadLightbox(document.querySelectorAll('#article-container micro:not(.no-lightbox)'))
  }
  ...
  const runJustifiedGallery = (container, data, config) => {
    ...
    const handleRenderComplete = e => {
      ...
      btf.loadLightbox(container.querySelectorAll('micro:not(.medium-zoom-image)'))
